...
def convert_pdf_to_txt(pdffilename: str, outfilename: str):
    with open(outfilename, "wt", encoding="utf-8") as md:
        for i, page in enumerate(pdf_pages(pdffilename)):
            if (i + 1) % 10 == 0:
                print(f"[i] Processing page {i + 1:d}")
            md.write(page + "\n")
...
if __name__ == "__main__":
    ...
    # Run the script
    convert_pdf_to_txt(args.pdffile, outfile)
